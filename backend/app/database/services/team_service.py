...
def get_team_member_relation_by_id(
    ...
    team_id: int = query.team_id
    return (
        db.query(UserToTeamModel)
        .filter(UserToTeamModel.team_id == team_id, UserToTeamModel.user_id == user_id)
        .first()
    )
...
def get_user_teams(db: Session, query: team_schemas.QueryAllUsersTeams) -> List[TeamModel]:
    ...
    return (
        db.query(TeamModel)
        .join(UserToTeamModel)
        .filter(UserToTeamModel.user_id == user_id)
        .all()
    )
