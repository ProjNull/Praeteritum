...
def get_all_team_member_relations(db: Session, query: team_schemas.QueryTeam) -> List[UserToTeamModel]:
    ...
    return db.query(
                ...
                UserToTeamModel.team_id == team_id
            ).all()


def get_team_member_relation_by_id(
    db: Session, query: team_schemas.QueryTeamMember
) -> UserToTeamModel:
    ...
    return (
            ...
            .filter(
                    UserToTeamModel.team_id == team_id, 
                    UserToTeamModel.user_id == user_id
                )
            .first()
        )
...
def get_user_permission_level_in_team(db: Session, query: team_schemas.QueryTeamMember) -> TeamPermissionLevel:
    team_id: int = query.team_id
    user_id: str = query.user_id
    return get_team_member_relation_by_id(
        db, 
        team_schemas.QueryTeamMember(
                        team_id=team_id, 
                        user_id=user_id
                    )
                ).permission_level
...
def set_user_permission_level_in_team(db: Session, query: team_schemas.TeamMemberUpdate):
    ...
    get_team_member_relation_by_id(
        db, 
        team_schemas.QueryTeamMember(
                        team_id=team_id, 
                        user_id=user_id
